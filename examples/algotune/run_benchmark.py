...
Examples:
  ...
  python examples/algotune/run_benchmark.py --output my_results.json
    ...
    parser.add_argument(
        "--config", type=str, help="Custom config file to use instead of task default configs"
    )
...
def parse_task_results(task_dir: Path, task_name: str, runtime_seconds: float) -> dict[str, Any]:
    ...
    if info_file.exists():
        try:
            ...
            return {
                "status": "failed",
                "error": f"Failed to parse best_program_info.json: {e!s}",
                "runtime_seconds": runtime_seconds,
                "speedup": None,
            }
    else:
