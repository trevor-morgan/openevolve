...
class AlgoTuneTaskAdapter:
    ...
    def __init__(self, algotune_path: str | None = None, task: str | None = None):
        """
        ...
        if self.task is not None:
            if self.task not in self.available_tasks:
                raise ValueError(
                    f"Task '{self.task}' not found. Available tasks: {list(self.available_tasks.keys())}"
                )
    ...
    def get_task_description(self, task_name: str) -> str:
        ...
        if task_name not in self.available_tasks:
            raise ValueError(
                f"Task '{task_name}' not found. Available tasks: {list(self.available_tasks.keys())}"
            )
    ...
    def _extract_task_class_info(self, task_name: str) -> dict[str, Any]:
        ...
        # Read the task file
        with open(task_info["task_file"]) as f:
            task_code = f.read()
                    ...
                    for item in node.body:
                        if isinstance(item, ast.FunctionDef) and item.name in [
                            ...
                            try:
                                ...
                                body_lines = []
                                def_indent = len(method_source_lines[0]) - len(
                                    method_source_lines[0].lstrip()
                                )
                                signature_end = 0
                                ...
                                if body_lines:
                                    ...
                                    if item.name == "solve":
                                        ...
                                        class_info["is_solution_method"] = (
                                            "            # Placeholder for is_solution method\n            pass"
                                        )
                            except Exception:
                                if item.name == "solve":
                                    class_info["solve_method"] = (
    ...
    def _harmonize_solve_and_is_solution(
        self, solve_method: str, is_solution_method: str, task_name: str
    ) -> tuple:
        """
        ...
        if "isinstance(proposed_list, list)" in harmonized_is_solution:
            harmonized_is_solution = harmonized_is_solution.replace(
                "isinstance(proposed_list, list)", "isinstance(proposed_list, (list, np.ndarray))"
            )

        # Fix error messages to reflect the change
        harmonized_is_solution = harmonized_is_solution.replace(
            "'transformed_image' is not a list.", "'transformed_image' is not a list or array."
        )
...
"""
            ...
            harmonized_is_solution = re.sub(
                pattern, r"\1" + conversion_code, harmonized_is_solution
            )
...
"""
    ...
    def _generate_initial_program(self, task_name: str) -> str:
        ...
        docstring_description = docstring_description.replace(
            "\\\\xauthentication", "\\\\x00authentication"
        )
...
'''
    ...
    def _generate_config(self, task_name: str) -> str:
        ...
        clean_description = re.sub(
            r"\\(\w+)(?:\{[^}]*\})?", replace_latex_command, clean_description
        )
        ...
        indented_description = "\n".join(
            "    " + line if line.strip() else "" for line in clean_description.split("\n")
        )
...
"""
    ...
    def _generate_task_specific_method(
        self, task_name: str, solve_method: str, class_info: dict[str, Any]
    ) -> str:
    ...
    def _generate_generic_method(
        self, task_name: str, problem_keys: list[str], return_type: str
    ) -> str:
        ...
        for key in problem_keys:
            ...
            validation_lines.append(
                f"                logging.error(f\"Problem must contain '{key}' key\")"
            )
            validation_lines.append(
                f'                raise ValueError(f"Missing required key: {key}")'
            )

        validation_code = (
            "\n".join(validation_lines)
            if validation_lines
            else "            # No specific validation needed"
        )
...
{return_code}"""
    ...
    def create_task_files(self, task_name: str, output_dir: str | None = None) -> str:
        """
