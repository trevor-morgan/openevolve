...
class MLIRAttentionEvaluator:
    ...
    def analyze_ir_complexity(self, mlir_content):
        ...
        for line in lines:
            ...
            if any(
                mem_op in stripped
                for mem_op in ["memref.load", "memref.store", "tensor.extract", "tensor.insert"]
            ):
                metrics["memory_ops"] += 1

            if any(
                arith_op in stripped
                for arith_op in ["arith.addf", "arith.mulf", "arith.divf", "arith.subf"]
            ):
    ...
    def estimate_performance_from_ir(self, optimized_metrics, baseline_metrics, params):
        ...
        arith_ratio = optimized_metrics["arithmetic_ops"] / max(
            baseline_metrics["arithmetic_ops"], 1
        )
    ...
    def apply_optimizations(self, mlir_content, params):
        ...
        unroll_factor = params.get("unroll_factor", 1)
        if unroll_factor > 1:
            passes.append("func.func(affine-loop-unroll)")
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".mlir", delete=False) as input_file:
            ...
            try:
                ...
                return None, f"Optimization error: {e!s}", None
            finally:
    ...
    def evaluate(self, optimize_attention_input):
        ...
        try:
            ...
            if isinstance(optimize_attention_input, str):
                if optimize_attention_input.startswith(
                    "/tmp/"
                ) and optimize_attention_input.endswith(".py"):
                    print(f"Loading code from: {optimize_attention_input}")
                    with open(optimize_attention_input) as f:
                        code = f.read()
            ...
            if self.baseline_mlir is None:
                ...
                print(
                    f"Baseline metrics: {self.baseline_metrics['operations']} ops, {self.baseline_metrics['loops']} loops"
                )

            # Apply optimizations
            optimized_mlir, error, compile_time = self.apply_optimizations(
                self.baseline_mlir, params
            )
            if error:
            ...
            print(
                f"Optimized metrics: {optimized_metrics['operations']} ops, {optimized_metrics['loops']} loops"
            )

            # Estimate performance using IR analysis
            print("Using sophisticated IR analysis for performance estimation...")
            result = self.estimate_performance_from_ir(
                optimized_metrics, self.baseline_metrics, params
            )
