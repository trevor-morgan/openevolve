...
def test_initial_program():
    ...
    try:
        result = subprocess.run(
            [sys.executable, "initial_program.py"], capture_output=True, text=True, timeout=10
        )
...
def test_evaluator():
    ...
    try:
        result = subprocess.run(
            [sys.executable, "evaluator.py", "initial_program.py"],
            capture_output=True,
            text=True,
            timeout=30,
        )
...
def main():
    ...
    tests = [test_mlir_file, test_initial_program, test_evaluator]
