...
def load_data(
    ...
    if app_name not in TASK_SCHEMA_MAP:
        raise ValueError(
            f"Task '{app_name}' not found in TASK_SCHEMA_MAP. Available tasks: {list(TASK_SCHEMA_MAP.keys())}"
        )
    ...
    try:
        ...
        raise OSError(
            f"Failed to load dataset '{app_name}' with split '{split}' from '{HUB_REPO_ID}'. Reason: {e}"
        )
...
if __name__ == "__main__":
    ...
    for task in ALL_TASKS:
        ...
        try:
            ...
            print(
                f"Train groups: {len(train_data)}. First group '{first_group_key}' shape: X={X_train.shape}, y={y_train.shape}"
            )
            ...
            if test_data:
                ...
                print(
                    f"Test groups: {len(test_data)}. First group '{first_test_key}' shape: X={X_test.shape}, y={y_test.shape}"
                )
            else:
